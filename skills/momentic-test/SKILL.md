...
base URL comes from `--url`, then the test's own `url`, then `env.BASE_URL`, so
a test that sets `url` keeps it in every environment. With no base URL the run
fails: "Cannot run test with no base URL and no BASE_URL variable defined in
its environment".
